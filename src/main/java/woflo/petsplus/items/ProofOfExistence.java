...
public class ProofOfExistence {
    ...
    private static String generateRoleEpithet(PetComponent petComp, MobEntity pet) {
        ...
        int index = PetRoleType.builtinIndex(roleId);
        if (index < 0) {
            index = 0;
        }

        long seed = pet.getUuid().getLeastSignificantBits() ^ index;

        int maxIndex = Math.max(0, (ROLE_EPITHETS.length / 4) - 1);
        int clampedIndex = Math.max(0, Math.min(index, maxIndex));
        int roleOffset = clampedIndex * 4;
